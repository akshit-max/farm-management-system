...
export const inventoryRepository = {
  ...
  create: async (data: any) => {
    ...
    const saveOffline = async () => {
      if (db) {
        await db.offline_inventory_items.add({
          local_id: localId,
          payload: payload,
          created_at: new Date(),
          updated_at: new Date(),
  ...
  update: async (id: string, data: any) => {
    let isLocal = false;
    if (db) {
      const localRecord = await db.offline_inventory_items.get(id);
      if (localRecord) isLocal = true;
    }

    if (isLocal) {
      const payload = { ...data, id };
      await db!.offline_inventory_items.update(id, { payload, updated_at: new Date(), sync_status: 'PENDING' });

      const queueTask = await db!.sync_queue.filter((t: any) => t.entity === 'INVENTORY_ITEM' && t.action === 'CREATE' && t.payload.id === id).first();
      if (queueTask) {
    ...
    if (!navigator.onLine) {
      // Create an offline update task for an online record
      const payload = { ...data, id };
      await db!.offline_inventory_items.add({
        local_id: id,
        payload: payload,
  ...
  delete: async (id: string) => {
    ...
    if (isLocal) {
      await db!.offline_inventory_items.delete(id);
      const queueTask = await db!.sync_queue.filter((t: any) => t.entity === 'INVENTORY_ITEM' && t.action === 'CREATE' && t.payload.id === id).first();
      if (queueTask) {
    ...
    if (!navigator.onLine) {
      await db!.offline_inventory_items.add({
        local_id: id,
        payload: { id, deleted_at: new Date() },
        created_at: new Date(),
        updated_at: new Date(),
        sync_status: 'PENDING'
      });
