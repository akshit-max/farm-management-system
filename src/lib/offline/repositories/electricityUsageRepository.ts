...
export const electricityUsageRepository = {
  getAll: async () => {
    ...
    if (navigator.onLine) {
      try {
        const res = await fetch(`/api/electricity-usage?t=${Date.now()}`);
        if (res.ok) {
    ...
    if (db) {
      const offlineRecords = await db.offline_electricity_usages.orderBy('created_at').reverse().toArray();
  ...
  create: async (data: any) => {
    ...
    const saveOffline = async () => {
      if (db) {
        try {
          await db.offline_electricity_usages.add({
            local_id: localId,
            payload: payload,
            created_at: new Date(),
            updated_at: new Date(),
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error("Failed to record electricity usage");
      }
  ...
  update: async (id: string, data: any) => {
    let isLocal = false;
    if (db) {
      const localRecord = await db.offline_electricity_usages.get(id);
      if (localRecord) isLocal = true;
    }

    if (isLocal) {
      const payload = { ...data, id };
      await db!.offline_electricity_usages.update(id, { payload, updated_at: new Date(), sync_status: 'PENDING' });
      
      const queueTask = await db!.sync_queue.filter((t: any) => t.entity === 'ELECTRICITY_USAGE' && t.action === 'CREATE' && t.payload.id === id).first();
      if (queueTask) {
    ...
    if (!navigator.onLine) {
      if (db) {
        const existingUpdate = await db.sync_queue.filter((t: any) => t.entity === 'ELECTRICITY_USAGE' && t.action === 'UPDATE' && t.payload.id === id).first();
        if (existingUpdate) {
    ...
    const res = await fetch(`/api/electricity-usage/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
    ...
    if (!res.ok) {
      ...
      try { errJson = await res.json(); } catch(e) {}
      if (errJson) throw errJson;
      throw new Error("Failed to update electricity usage");
    }
  ...
  delete: async (id: string) => {
    ...
    if (isLocal) {
      await db!.offline_electricity_usages.delete(id);
      const queueTask = await db!.sync_queue.filter((t: any) => t.entity === 'ELECTRICITY_USAGE' && t.action === 'CREATE' && t.payload.id === id).first();
      if (queueTask) {
    ...
    const res = await fetch(`/api/electricity-usage/${id}`, { method: "DELETE" });
    if (!res.ok) {
      let errJson;
      try { errJson = await res.json(); } catch(e) {}
      if (errJson) throw errJson;
      throw new Error("Failed to delete electricity usage");
    }
    return { success: true, offline: false };
  }
};
