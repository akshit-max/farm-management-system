...
export async function PUT(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const parsedData = updateSupplierSchema.parse(body);
    ...
    await logAudit(session.user.id, farmId, "UPDATE", "Supplier", params.id);
...
export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await logAudit(session.user.id, farmId, "DELETE", "Supplier", params.id);
    return NextResponse.json({ success: true });
  } catch (error) {
