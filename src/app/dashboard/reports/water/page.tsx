...
export default function WaterPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      let url = `/api/reports/water?period=${period}`;
    ...
    try {
      const res = await fetch(`/api/reports/export/${format}`, {
        ...
        body: JSON.stringify({
          ...
          columns: [{ header: 'Date', key: 'date' }, { header: 'Room', key: 'room' }, { header: 'Consumption (L)', key: 'consumption' }, { header: 'Cost (₹)', key: 'cost' }],
          data: data?.rows || []
        })
      });
      ...
      a.href = url;
      a.download = `Water_Usage_Report.${format === 'excel' ? 'xlsx' : 'pdf'}`;
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-gray-50 text-gray-800 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Consumption (L)</th>
