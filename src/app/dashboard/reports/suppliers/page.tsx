...
export default function SuppliersPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      let url = `/api/reports/suppliers?period=${period}`;
    ...
    try {
      const res = await fetch(`/api/reports/export/${format}`, {
        ...
        body: JSON.stringify({
          ...
          columns: [{ header: 'Supplier Name', key: 'supplier', width: 200 }, { header: 'Linked Feeds', key: 'linkedFeedTypes' }, { header: 'Usage Frequency', key: 'usageFreq' }],
          data: data?.rows || []
        })
      });
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-gray-50 text-gray-800 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Linked Feeds</th>
                <th className="px-6 py-4 font-semibold">Usage Frequency</th>
              </tr>
            </thead>
