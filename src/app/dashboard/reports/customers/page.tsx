...
export default function CustomersPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      let url = `/api/reports/customers?period=${period}`;
    ...
    try {
      const res = await fetch(`/api/reports/export/${format}`, {
        ...
        body: JSON.stringify({
          ...
          columns: [{ header: 'Customer Name', key: 'customer', width: 200 }, { header: 'Invoice Count', key: 'count' }, { header: 'Last Purchase', key: 'lastDate' }, { header: 'Revenue (₹)', key: 'revenue' }, { header: 'Outstanding (₹)', key: 'outstanding' }],
          data: data?.rows || []
        })
      });
      ...
      a.href = url;
      a.download = `Customer_Revenue_Ranking.${format === 'excel' ? 'xlsx' : 'pdf'}`;
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Customer Revenue Ranking</h1>
        </div>
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-gray-50 text-gray-800 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold">Invoice Count</th>
