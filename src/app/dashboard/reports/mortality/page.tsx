...
export default function MortalityPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      let url = `/api/reports/mortality?period=${period}`;
    ...
    try {
      const res = await fetch(`/api/reports/export/${format}`, {
        ...
        body: JSON.stringify({
          ...
          columns: [{ header: 'Date', key: 'date' }, { header: 'Batch', key: 'batch' }, { header: 'Category', key: 'category' }, { header: 'Quantity', key: 'quantity' }, { header: 'Reason', key: 'reason' }],
          data: data?.rows || []
        })
      });
      ...
      a.href = url;
      a.download = `Mortality_Report.${format === 'excel' ? 'xlsx' : 'pdf'}`;
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Mortality Report</h1>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm h-[300px]">
          ...
          ) : <EmptyState title="No trend data" description="No mortalities recorded" />}
        </div>
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm h-[300px]">
          ...
          ) : <EmptyState title="No category data" description="No mortalities recorded" />}
        </div>
      </div>
