...
export function ExpenseTable({ data, onEdit, onRefresh, canMutate }: { data: any[]; onEdit: (u: any) => void; onRefresh: () => void; canMutate: boolean }) {
  ...
  const columns = useMemo(() => {
    const baseColumns: any[] = [
      ...
      columnHelper.accessor("amount", {
        header: "Amount",
        cell: (info) => <span className="font-bold text-status-danger">₹{Number(info.getValue()).toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
      }),
    ];
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-40 text-center">
                  <div className="flex flex-col items-center justify-center text-gray-500">
                    ...
                    <p className="text-sm">Record manual expenses here.</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
